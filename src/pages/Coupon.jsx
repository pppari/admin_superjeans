...
const CouponTable = () => {
  ...
  const handleToggleActive = async (id, currentStatus, record) => {
    if (record.valid_to && new Date(record.valid_to) < new Date()) {
      messageApi.warning('คูปองนี้หมดอายุแล้ว');
      return;
    }
  ...
  const handleDeleteCoupon = async () => {
    try {
      ...
      messageApi.open({ type: 'success', content: 'ลบส่วนลดสำเร็จ' });
    } catch (error) {
      messageApi.open({ type: 'error', content: 'ลบส่วนลดล้มเหลว' });
      console.error(error);
    }
  };
          ...
          },
          {
            ...
            render: (_, record) => (
              <Switch
                ...
                onChange={() =>
                  handleToggleActive(record._id, record.isActive, record)
                }
              />
